...
def loadDataSet():
    ...
    matLabel=mat(dataLabel).transpose()  # numpy List 转mat矩阵
    print(type(matLabel))
...
def graAscent(dataMatrix,matLabel):
    ...
    for i in range(num):
        ...
        w=w-alpha*matMatrix.transpose()*error  # w = w +a*error*x(T)
    return w

def stocGradAscent0(dataMatrix,matLabel):
    m,n = shape(dataMatrix)
    alpha=0.01
    w=ones(n)
    matMatrix=mat(dataMatrix)
    for i in range(m):
        h = sigmoid(sum(matMatrix[i]*w))
        error = matLabel-h
        w = w + alpha*matMatrix[i]*error
...
def stocGraAscent(dataMatrix,matLabel):
    ...
    print(w)
    alpha=0.001
    num=2000  #这里的这个迭代次数对于分类效果影响很大，很小时分类效果很差
    for i in range(num):
        for j in range(m):
...
if __name__ == '__main__':
    ...
    # weight=stocGradAscent0(dataMatrix,matLabel)
    # weight=stocGraAscent(dataMatrix,matLabel)
