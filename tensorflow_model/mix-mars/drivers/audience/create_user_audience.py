...
DMP_SRV_AUDIENCE_DETAIL_ADD_PARTITION = '''INSERT INTO TABLE dmp_srv_audience_detail 
    ...
    '''
DMP_SRV_AUDIENCE_DETAIL_ADD_PARTITION_MIX_UID = '''INSERT INTO TABLE dmp_srv_audience_detail
    PARTITION (audienceID)
    SELECT b.mix_uid, b.flag_id, b.flag, b.province, a.audienceID
    FROM %(tmp_table)s a LEFT OUTER JOIN dmp_um_user_rel_dt_tmp_%(audience_id)s b ON lower(a.mix_uid)=lower(b.mix_uid)
    WHERE b.mix_uid IS NOT NULL
    '''
...
def HiveExe(hql, data_base, queue):
    "客户端运行hql"
    cmd = "hive -e \" use "+data_base+";set mapreduce.job.queuename="+queue+";set mapreduce.map.memory.mb=2048;set mapreduce.reduce.memory.mb=5120;set hive.groupby.skewindata=true;add jar ../report/mix-dmp-udf-1.0.0.jar;create temporary function md5 as 'com.ai.hive.udf.util.Md5UDF';create temporary function Sha1 as 'com.ai.hive.udf.util.Sha1UDF';"
    for sql in hql:
        cmd = cmd + sql + ";"
...
def dropTmpTables():
    ...
    for tt in tmp_tables:
        ...
        HiveExe2([hs], SORUCE_DATABASE, 'default')
        print '%s临时表已删除' % tt
    return 0
...
def uploadUserFilesToHdfs(file_name, audience_id, user_name):
    #定义文件路径
    hive_partition_path_prefix = "/user/hive/warehouse/dmptest_user_dir/zhanghd/dmp_srv_audience_in/"
...
#    user_file_path_prefix = "/data1/user/zhanghd/mix-mars-0.0.1/user_files/"
    ...
    status = HiveExe([add_partition], SORUCE_DATABASE, 'default')
    if status == 0:
        print '添加用户ID分区完成'
        return 0
    else:
...
def getUserAudienceValue(audience_id):
    ...
    flag_mapping = {'md5imei':'\'md532_imei\'','imei':'\'std_imei\'', 'idfa':'\'idfa\'', 'mac':'\'mac\'',
                  ...
                  'sha1idfa':'\'sha1_idfa\'','sha1mac':'\'sha1_mac\'','mixuid':'\'mix_uid\'' }
    
    print '开始统计自有人群输入UV...'
    user_in_uv_tmp = USER_IN_UV % audience_id
    print "user_in_uv_tmp",user_in_uv_tmp
   ...
   # if status == 0:
    ...
    elif(flag.__contains__("mix_uid")):
    ...
    print "user_rel_tab_tmp",user_rel_tab_tmp
    ...
    print "data_add_partition",data_add_partition
