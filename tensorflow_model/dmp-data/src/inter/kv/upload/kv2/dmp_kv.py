...
#   紧急流程：      xxx.py beijing dmp_uc_tags_bh 2014110110 2014112523
#**--------------------------------------------------------------------------------------------------------
#**
#***********************************************************************************************************
...
startTime = sys.argv[5]  # 开始时间
endTime = sys.argv[6]  # 结束时间
v_x = sys.argv[7] # 用户批次

#为了多个表能够同时上传
name = name +"_"+tablename + "_" + v_x

#校验格式并构建查询条件
cond = ''
if(len(startTime) == 10 and len(endTime) == 10):
    target_tablename = "dmp_upload_inter_tab_bh_2"
...
#===============================================================================================
    ...
    if(type=='none'):
		...
		if(type_value == 'none'):
			...
			cond_type = " and "+type+" in ("+type_value+")"
...
# 分隔符加载
    ...
    hivesql.append("create temporary function UDFConcat_ws as 'com.ai.hive.udf.util.ConcatWsUDF'")
    hivesql.append('''
	...
	from(
		from(
			select '%(tablename)s' as tablename,%(userid)s as userid,%(type)s
				...
				and ascii(substr(%(userid)s,0,1))%%1=%(v_x)s  
				cluster by userid
			) a 

			map a.tablename,a.userid,%(c_type)s a.value,%(datatime)s
				using 'python mapper.py' as user_id,value,%(datatime)s
		) b

		insert overwrite table %(HIVE_DATABASE)s.%(target_tablename)s
			partition (%(partition_str)s,tab,province)
			reduce  concat_ws('_','%(v_x)s','%(pri)s'),b.value,'%(startTime)s',
			concat_ws('_','%(tablename)s','%(v_x)s'),'%(province)s',b.user_id 
	...
	insert overwrite table %(HIVE_DATABASE)s.%(target_tablename)s
	partition (%(partition_str)s,tab,province)
	select key,value,%(partition_str)s,'%(tablename)s_%(v_x)s_num' as tab,'%(province)s'
	from(
		select split(key,'-')[0] as key,count(1) as value,%(partition_str)s 
			from %(HIVE_DATABASE)s.%(target_tablename)s
			where province='%(province)s' and hour_id='%(startTime)s' 
			and tab=concat_ws('_','%(tablename)s','%(v_x)s')
