...
if(len(startTime) == 10 and len(endTime) == 10):
    target_tablename = "dmp_upload_inter_tab_bh_1"
...
#===============================================================================================
    ...
    hivesql.append('''
		...
		( key string, user_id string,value string)
    ...
    hivesql.append('''
		...
		from(
			...
			insert overwrite table %(HIVE_DATABASE)s.%(target_tablename)s
				...
				reduce '%(pri)s',b.value,%(datatime)s,'%(tablename)s','%(province)s',b.user_id
				using 'python reducer.py' as key,user_id,value,%(partition_str)s,tablename,province
		''' % vars())
    HiveExe(hivesql, name, startTime)
	...
	insert overwrite table %(HIVE_DATABASE)s.%(target_tablename)s
	partition (%(partition_str)s,tab,province)
	select key,user_id,value,%(partition_str)s,'%(tablename)s_num' as tab,'%(province)s'
	from(
		select split(key,'-')[0] as key,'user_cnt' as user_id,count(1) as value,%(partition_str)s 
