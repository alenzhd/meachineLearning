...
#自定义变量声明---源表声明
#===========================================================================================
    source_tb_stdtags = "dmp_online.dmp_ud_user_stdtags_bd"
    source_tb_catags = "dmp_online.dmp_ud_user_catags_bd"
    source_tb_relobj = "dmp_online.dmp_um_user_relobj_bd"
    source_tb_rel = "dmp_online.dmp_um_user_rel_dt"
...
#================================================================================
    ...
    hivesql.append('''
        ...
        select tb2.dev_id,tb1.province, tb2.rel_province
        from(
            select mix_uid, tags, name, weight, type_id as tag_type,province
            from(
                select a.mix_uid, a.tags, b.name, a.weight, if(a.type_id = 'b', 'site', if(a.type_id = 'c', 'app', 'kw')) as type_id,province
                from(
                    select mix_uid, tags, type_id,weight,province
                ...
                select a.mix_uid, a.tags, b.name, a.weight, 'cont' as type_id,province
                from(
                    select mix_uid, tags, weight,province
                ...
                select a.mix_uid, a.relobj_id as tags, b.name, a.weight, 'pubnum' as type_id,province
                from(
                    select mix_uid, relobj_id, weight,province
        ...
        join(
           select b.mix_uid,a.dev_id,b.province as rel_province from 
            (select dev_id from zhanghd.dmp_srv_id_tags_bd_test1 group by dev_id)a join
            (select mix_uid,lower(if(flag='idfa',Md5UDF(flag_id),flag_id)) as dev_id,province from dmp_online.dmp_um_user_rel_dt
            where day_id=20171105 and flag in ('md532_imei','idfa') )b on a.dev_id = b.dev_id
        )tb2
        on (tb1.mix_uid = tb2.mix_uid )
        group by tb2.dev_id, tb1.tags, tb1.name, tb2.id_type, tb1.tag_type,tb1.province
    ''' % vars())
    HiveExe(hivesql, name, dates)
