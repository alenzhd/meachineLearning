...
def HiveExe(hql,name,dates):
    cmd = "hive -e \" use "+HIVE_DATABASE+";set mapreduce.map.failures.maxpercent=10;set mapreduce.reduce.failures.maxpercent=10;set  mapred.job.reuse.jvm.num.tasks=0;set mapreduce.job.queuename="+QUEUE+";"
    for sql in hql:
        cmd = cmd + sql + ";"
        print time.strftime( ISOTIMEFORMAT, time.localtime() )
    cmd =cmd + "\""
    
    maxCount = 3  #max runing count
    status = 1
    count = 1
    while status != 0 and count <= maxCount:
        ...
        if status == 0 :
            return 0
        tmpLog = 'Runing Count:'+str(count)+'....'
        print tmpLog
        count = count + 1
    else:
    	tmpLog = 'Runing Count over the Max Count limit '+str(maxCount)+',exit'
        print tmpLog
        return 1
