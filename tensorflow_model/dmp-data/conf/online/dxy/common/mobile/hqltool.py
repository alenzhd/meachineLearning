...
v_python_path = []
v_python_path.append(mix_home+'/conf')
...
def HiveExe1(hql,name,dates):
    cmd = "hive -e \" use "+HIVE_DATABASE+";set mapreduce.job.queuename="+QUEUE+";set mapreduce.map.memory.mb=2048;set mapreduce.reduce.memory.mb=5120;set mapreduce.map.java.opts=-Xmx1900m;set mapreduce.reduce.java.opts=-Xmx4900m;set hive.groupby.skewindata=true;"
    for sql in hql:
        cmd = cmd + sql + ";"
...
def HiveExe(hql,name,dates):
    cmd = "hive -e \" use "+HIVE_DATABASE+";set mapreduce.job.queuename="+QUEUE+";set mapreduce.map.memory.mb=2048;set mapreduce.reduce.memory.mb=5120;set hive.groupby.skewindata=true;"
    for sql in hql:
        cmd = cmd + sql + ";"
        print time.strftime( ISOTIMEFORMAT, time.localtime() )
    cmd =cmd + "\""
...
def Start(name,dates):
    filename = PROVIDER+'_'+PROVINCE+'_'+ NET_TYPE+'_'+name+'_'+dates
    ...
    if e is False:
        ...
        print "请删除文件锁：rm "+lock_path+filename
        sys.exit(1)
        #return 1
...
def Except(name,dates,e):
    filename = PROVIDER+'_'+PROVINCE+'_'+ NET_TYPE+'_'+name+'_'+dates
    print e
    isFile = os.path.isfile(r'%s' % (lock_path+filename))
    if isFile is False:
		print '未成功启动程序！'
    else:
