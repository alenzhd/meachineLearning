...
def HiveExe1(hql,name,dates):
    cmd = "hive -e \" use "+HIVE_DATABASE+";set mapreduce.job.queuename="+QUEUE+";set mapreduce.map.memory.mb=2048;set mapreduce.reduce.memory.mb=5120;set mapreduce.map.java.opts=-Xmx1900m;set mapreduce.reduce.java.opts=-Xmx4900m;"
    for sql in hql:
        cmd = cmd + sql + ";"
...
def HiveExe(hql,name,dates):
    cmd = "hive -e \" use "+HIVE_DATABASE+";set mapreduce.job.queuename="+QUEUE+";set mapreduce.map.memory.mb=2048;set mapreduce.reduce.memory.mb=5120;"
    for sql in hql:
        cmd = cmd + sql + ";"
        print time.strftime( ISOTIMEFORMAT, time.localtime() )
    cmd =cmd + "\""
